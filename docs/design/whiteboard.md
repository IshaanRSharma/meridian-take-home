...
## 4. How a field gets filled

Five ways, strictly ordered by cost and reliability. **The rule is never to ask
for what a lower tier can get** — every tier skipped is a question the process
owner does not have to answer.

| | source | needs a model? | example |
|---|---|---|---|
| **derive** | the board already knows | no | `capabilities` from `channel` + `effect`; the certificate count from the invoice's batches |
| **pick** | it must resolve to something real | no | any field path, any outcome name — a dropdown, never a text box |
| **fill** | the owner already said it, in prose | yes, per card | *"email the receiving supervisor with the invoice number"* → four fields |
| **lint** | they did not say it, and it is blank | no | *"Nobody is named as receiving this"* |
| **review** | they said everything, and a choice could still be wrong | yes | *"does anyone else need to know when it is a controlled substance?"* |

**The reviewer is the last resort, not the front door.** A recipient is a blank
field, so lint catches it deterministically and the finding's own text is the
question — no model is involved in the pre-alert board's most-cited gap. If the
reviewer is doing a lot of work, the tiers above it are underbuilt.

### Where fill is allowed to draft

The recorded rule is *fill handles description, pickers handle reference*
(`Claude.md` §7). Stated by its actual reason it is sharper:

> **Fill may draft anything lint or bind check can verify. Pickers own anything
> where a wrong-but-valid answer is possible.**

| a bad… | caught by |
|---|---|
| field path | lint — every reference has to resolve |
| role | `bind check` — it reports the role as unbound |
| criterion | **nothing.** It resolves, it is well formed, and it tests the wrong thing |

That last row is why `criteria`, `outcomes`, `inputs`, `captures` and `scope`
are pickers. Not because they are references — because a plausible wrong answer
there is undetectable, which is the one failure mode worth designing against.

### Fill is a typing accelerator, never an authority

It writes into the same hand-written form the owner can edit, which is the
practical reason [`Claude.md` §31](../../Claude.md) refuses autogenerated forms:
the fields have to be stable enough to type into directly. So the same value can
arrive three ways — from the prose box, typed into the field, or by clicking a
lint finding and landing on the input — and nothing downstream can tell which.
`Finding.kind` is `"field" | "structure"` precisely so the interface knows
whether to focus an input or highlight the canvas.

### Fill is lossy on purpose, and the residual is kept

It extracts only what lints. Everything else stays in `instructions` and reaches
codegen as prose:

> *"Nursys for most states, but California and Texas have their own portals. We
> search by licence number and the licensee's last name."*

`system: "state medical board"` is the part that lints. The rest is the binding
brief — one capability, three backends, state routing — and a fill that discards
the sentence after extracting the field destroys it. **Structured fields exist to
make linting possible; everything else rides in prose, because the consumer is a
language model, not a parser.**

Not built. `fill` has no code yet; the decision is recorded in `Claude.md` §7.
