...
| Every card carries `name`; three fields cut | `event.source`, `event.resulting_state` and `action.completion_criteria` came from the Day 1 PRD and no consumer reads any of them — §28's own test sends them to prose. `check.question` became `name`, so all four cards label themselves the same way. `action.performed_by` survives but is only required for `effect: decide`, where who decides is load-bearing — and it is a `RoleRef`, not free text, so an identity never reaches the checksummed spec and the bindings file has something to key on. |
...
| Severity is required, never defaulted | a default makes every field added later a freeze gate nobody chose. `blocking` has to keep meaning *no correct agent can be generated without this*, and with the literal spelled at every call site, `grep '"blocking"'` is the complete list of gates in the system. Flipping the default to `important` would have been the gentler fix and the weaker one — it hides the decision instead of forcing it. |
| A finding is a fact, not a prompt | `findings()` has three consumers and only the reviewer speaks to the process owner, capped at six a round. Nothing blocks while someone is drawing; blocking bites once, at Submit. |
| Four filters between a finding and a human | the board is authored from prose, so an extraction miss would otherwise become a question about something the owner already said. auto-fix → re-read the prose → structural gate → review round (cap 6). Each stage attempts rather than labels, so improving the extractor quietly reduces the questions. |
| **A missing value is never blocking** | `blocking` means the drawing does not work as a process — a reference that does not resolve, an outcome with no line, a step it stops at without saying so. Provable, visible on the canvas, closable by anyone. Who receives a report or which system a log goes to is knowledge only the process owner has, so it is the reviewer's question and never a refusal handed to someone who came to draw a diagram. Enforced by a test over every config, not by intent. Two gates fall out: *enter review* needs the drawing to work, *freeze* needs that plus every question settled. |
| `Scope` is `per_case`, not `per_shipment` | the grain is the unit the correlation key identifies — a shipment here, an application in credentialing. A closed enum reaches the frozen spec and the generated code, so a value borrowed from one customer's process puts pharma logistics in the type system and leaves a credentialing board with no honest grain to pick. Free text carries the customer's nouns; enumerations carry ours, and ours have to be domain-free. Guarded by a test over every `Literal` in `primitives.py`. |
| A reference must be **declared**, not merely resolvable | every other reference rule asks whether a field exists. `references_are_declared` asks whether the step was handed the card it reads from — a different failure, and invisible to the others, because the field is real and the entity is on the board. The seed board had it: `report_coa_discrepancy` emailed an `invoice_no` from a document its `inputs` never named, which both reached codegen as an unpassed argument and silently cut that card out of the scope chain. |
| The reviewer payload carries whole configs | structure is what lint and the sweeps already interrogate. Every *semantic* question comes from the prose the owner typed — the subject lines they quoted, the operator they picked, the aside in `instructions` — and none of it is derivable from the graph. A skeleton of keys and edges is a board to review shapes on, not a process to review. |
| Unset fields never reach the generator | the consumer reads one card's entry to write one file, and a field nobody filled says nothing. `exclude_none` on both serialisations; 16% of the seed's spec payload was nulls and the proportion grows with the schema, not the board. |
| The email is an Entity, not a match condition | it is parsed, not merely matched: its sender is who a report replies to and its attachments are what get classified. Left as a `match_condition` string none of that is addressable, and *"whoever sent the pre-alert"* cannot be expressed as a recipient at all. |
| A decision is a step that has to reach someone | `effect: decide` asked who decides and never how to ask them, so a three-day approval compiled to a task with no address. Both effects that put a person on the other end — `notify` and `decide` — now require `recipients` and `channel`, and both derive the same delivery capability from it. |
| An Action deadline needs a named outcome | `EventConfig` guarded this and `ActionConfig` did not, so an SLA's third way out — the clock running down — had no outcome, therefore no edge, therefore nothing `outcomes_are_wired` could see. The board linted clean and the generated code had a timer firing into nothing. |
| Reading a document is codegen's call, not the spec's | the generator already has the entity and its field schema; how those fields come out of what arrived is an implementation decision, and those are the ones it is allowed to make. Nothing on a board says whether a thing is a file, so deriving `doc.extract` meant guessing — and it guessed wrong on every process without documents, demanding a PDF reader for a five-field queue row. |
| A statement about a *thing* has its own home | the transposition walks steps, so an answer anchored on an entity — how to recognise a certificate, how a batch number is written — reached no card and was dropped at the freeze in silence. `entity_context` sits beside `edge_context` for the identical reason, and it is what a generator writing an extraction schema actually needs. |
