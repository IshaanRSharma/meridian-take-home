...
container code and an air waybill is not one. Today such a message falls out of
`group_by_shipment` and nothing anywhere says so — the trigger would appear to
work and would quietly ignore a real shipment. So an uncorrelatable message
becomes a result in its own right, naming what arrived and why it could not be
keyed.
...
three different units of work, and the unit of work is the one thing the repair
loop is not allowed to decide — every count stays internally consistent and
wrong together. That is a question for the process owner.
"""
...
def uncorrelated_in(messages: Collection[mail.Message]) -> tuple[Uncorrelated, ...]:
    ...
    mailbox or a workflow. This is the whole of the bug being fixed: the same
    predicate `group_by_shipment` uses to *exclude* a message, used here to
    *report* it.
    """
    return tuple(
        Uncorrelated(
            ...
            reason=(
                "matched the pre-alert subject rule and names no container number; "
                "the correlation key on this board is an ISO container code, and an "
                "air waybill is not one"
            ),
        )
